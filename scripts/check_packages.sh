...
# Resolution id 30 missing elsewhere). After dropping those ids, the
# remaining entries must be identical — anything else is drift.
interact_hash() {
  jq -S '[.interact.variables[] | select(.id != 25 and .id != 30 and .id != 35)]' "$1" \
    | md5sum | awk '{print $1}'
}
if [ "$(for f in pkg/Cores/*/interact.json; do interact_hash "$f"; done | sort -u | wc -l)" -ne 1 ]; then
  echo "DRIFT: interact.json differs across packages beyond the intentional ids (25/30/35):"
